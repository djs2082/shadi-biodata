...
https://shadibiodata.com ('Website'), including the related mobile site and mobile application
(hereinafter referred to as 'Platform').
3. The Platform is owned by Joshi Solutions, a company incorporated under the Companies Act,
1956 with its registered office at Crystal Hills Apartments, Baner, Pune (hereinafter referred to
...
Privacy Policy
Introduction
This Privacy Policy describes how Joshi Solutions and its affiliates (collectively "Joshi Solutions, we,
our, us") collect, use, share, protect or otherwise process your information/ personal data through our
website https://shadibiodata.com (hereinafter referred to as Platform). Please note that you may be
able to browse certain sections of the Platform without registering with us.We do not offer any product
...
any information. If you receive an email, a call from a person/association claiming to be Joshi Solutions
...
1. Cancellations will only be considered if the request is made 2 days of placing the order. However,
...
2. Joshi Solutions does not accept cancellation requests for perishable items like flowers, eatables,
...
determined the same at its own end. This should be reported within 2 days of receipt of products.
In case you feel that the product received is not as shown on the site or as per your expectations,
you must bring it to the notice of our customer service within 2 days of receiving the product. The
...
5. In case of any refunds approved by Joshi Solutions, it will take 7 days for the refund to be
processed to you.
...
registration. If there are any shipping cost(s) levied by the seller or the Platform Owner (as the case be), the same is not refundable.`;
  ...
  return (
    <div className="terms-page">
      {blocks.map((b, i) => {
        const t = b.trim();
        if (!t) return null;
        if (/^Terms & Conditions/i.test(t)) return <h1 key={i}>{t}</h1>;
        if (/^Privacy Policy/i.test(t)) return <h2 key={i}>{t}</h2>;
        if (/^Refund and Cancellation policy/i.test(t))
          return <h2 key={i}>{t}</h2>;
        if (/^Return Policy/i.test(t)) return <h2 key={i}>{t}</h2>;
        if (/^Shipping Policy/i.test(t)) return <h2 key={i}>{t}</h2>;
        return <p key={i}>{t}</p>;
      })}
    </div>
  );
